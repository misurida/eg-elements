...
.eg-input {
    ...
    .range-slider {
        ...
        .slider-dot {
            ...
            background: var(--color-primary);
            cursor: pointer;
            transition: box-shadow .2s, left .2s, opacity .2s;
        ...
        &.focus {
            .slider-dot {
                transition: all .2s;
                box-shadow: var(--input-box-hard-shadow);
            }
...
<template>
    <div
        ...
        <div class="eg-input-wrapper" :class="{'two-sided': type==='range'}">
            <div v-if="type==='range'" class="slider-panel" @click="hFocus">
                <input type="range" :class="{wide}" v-if="native || true" :min="_visibleMin" :max="_visibleMax" :step="step" @input="hInput" :value="value">
                <div class="range-slider" :class="{focus:focus}" v-else @mouseleave="hSliderOut" @mouseover="hSliderMove">
                    <div class="slider-line" @mouseover="hSliderMove"></div>
                    <div class="slider-dot" :style="dotStyle" @mousedown="hSlideDown" @mouseup="hSliderUp" @mouseover="hSliderMove"></div>
                </div>
            </div>
            <!-- input shell -->
            <div :class="{focus, 'input-shell':!buttonStyle}" @mousedown="hMouseDown">
                ...
                <template v-else>
                    ...
                    <div class="eg-inner" :class="{'no-edit':!editable, empty:_empty&&!emptyCross}" @click="hClick">
                        <!-- Left side -->
                        <div class="eg-icons-left-list" :class="{crossless:!_cross}" v-if="showLeftIcon">
                        ...
                        <div class="eg-icons-right-list" @click="hFakeClick" :class="{crossless:!_cross}" v-if="showRightIcon">
                            <div class="eg-main-icon-slot" v-if="(_cross||hasRightIcon)">
                                ...
                                <eg-icon v-if="(!_empty||emptyCross)&&_cross" class="eg-cross-icon" :flip="flipIcon" svg icon="scross" clickable @click="hCross" stop></eg-icon>
                            </div>
        ...
        <div class="eg-ghost-text" v-if="_flex">{{ value || placeholder }}</div>
...
export default {
    ...
    props: {
        ...
        visibleMin: {type: Number, default: null},
        visibleMax: {type: Number, default: null}
    },
    data() {
        return {
    ...
    methods: {
        ...
        hCross(e) {
            if(!this.disabled) {
                ...
                if(this.isNumber && this.emptyValue === undefined) {
            ...
            if(this._flex) {
                this.$nextTick(() => {
                    if(this.$el) {
                        ...
                        if(ghost) this.textWidth = ghost.offsetWidth ? ghost.offsetWidth + 10 : this.baseMin;
    ...
    computed: {
        ...
        _regexList() {
            if(this._type === "number") return [...this.regexList, { regex: /^([0-9.]*)$/, strong: true }];
        ...
        _type() {
            ...
            //if(this.type === "number" && !this.native) {
            //    return "text";
            //}
            return this.type;
        },
        _plusMinus() {
            if(this.native) {
                ...
                return this.plusMinus !== false;
            }
            return this.plusMinus;
        },
        showRightIcon() { return this.icons.length > 0 || this.hasRightIcon || this._cross || this.search || this._plusMinus; },
        ...
        _flex() { return this.type === "range" ? true : this.flex },
        inputStyle() {
            if(this._flex) {
        ...
        _visibleMin() { return this.visibleMin === null ? this.min : this.visibleMin },
        _visibleMax() { return this.visibleMax === null ? this.max : this.visibleMax },
    },
